...
\begin{equation}
\begin{aligned}
\mathcal{L} = \underbrace{\mathrm{CE}(f(x), y)}_{\text{supervised}} + \underbrace{\mathrm{CE}(f(x'), y)}_{\text{corrupted}} + \\ 
\underbrace{\lambda_t \cdot D_{\mathrm{KL}}\!\left(p(x) \,\|\, p(x')\right)}_{\text{consistency}},
\label{eq:loss}
\end{aligned}
\end{equation}
...
All three models use the same ResNet-18-CIFAR backbone, the same SGD optimiser, the same number of training epochs (100), and the same random seed. Figures~\ref{fig:baseline_training} and~\ref{fig:augmix_training} show the training and validation curves for the Baseline and AugMix models respectively. The Baseline exhibits a widening gap between training and validation loss after epoch 60, indicating overfitting to the clean training distribution. AugMix, by contrast, shows tightly coupled training and validation curves throughout all 100 epochs, with both loss and accuracy converging closely --- a behaviour attributable to the regularising effect of its JSD consistency objective across three augmented views.

\begin{figure}[t]
\centering
\includegraphics[width=0.95\columnwidth]{baseline_training.png}
\caption{Training and validation loss and accuracy curves for the Baseline ResNet-18 over 100 epochs. The growing gap between training and validation loss from epoch 60 onwards reflects overfitting to the clean CIFAR-10 distribution, consistent with the model's poor corruption robustness at test time.}
\label{fig:baseline_training}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=0.95\columnwidth]{augmix_training.png}
\caption{Training and validation loss and accuracy curves for the AugMix ResNet-18 over 100 epochs. The close alignment of training and validation curves throughout training reflects the regularising effect of AugMix's three-view JSD consistency objective, which discourages the model from memorising clean-image artefacts.}
\label{fig:augmix_training}
\end{figure}
...
\begin{table}[t]
\centering
\caption{Clean accuracy and mean corrupted accuracy (mCA) on CIFAR-10-C.  $\uparrow$ higher is better; }
...
Baseline        & 94.3\%       & 73.1\% \\
AugMix          & 95.3\%       & 86.2\% \\
CRT (Proposed)  & 93.7\%       & 88.9\% \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:main_results} reports clean accuracy on the CIFAR-10 test set and mean corrupted accuracy (mCA) averaged over all 15 corruption types and 5 severity levels on CIFAR-10-C. The Baseline achieves 94.3\% clean accuracy but degrades substantially on corrupted inputs, reflecting its sensitivity to distribution shift between the clean training distribution and corrupted test conditions. AugMix raises mCA to 86.2\% through its three-view JSD consistency objective, at a marginal cost to clean accuracy.

CRT achieves 88.9\% mCA --- a 15.8 percentage-point improvement over the Baseline --- while preserving clean accuracy at 93.7\%. This improvement is a direct consequence of the KL consistency objective: by penalising divergence between the model's predictions on clean and corrupted views of the same image during training, CRT explicitly reduces the model's sensitivity to the kinds of distribution shift it will encounter at test time. The model is trained to produce stable, class-consistent outputs even when the input is degraded, which translates directly into reduced accuracy loss under corruption. The warm-up schedule reinforces this by ensuring the KL pressure is applied only after the model has formed reliable class-discriminative representations, preventing the consistency objective from interfering with early feature learning.

On this controlled CIFAR-10-C evaluation, CRT achieves a higher mCA than AugMix (88.9\% vs.\ 86.2\%) while requiring only two forward passes per training step rather than three. It is important to note, however, that AugMix employs a broader and more diverse set of augmentation operations --- thirteen distinct image transformations combined stochastically --- whereas CRT uses a fixed pool of five corruption types at a single severity level. AugMix's diversity advantage may become more pronounced on benchmarks that include corruption types not represented in the CRT training pool, or at larger scale. The result here should therefore be interpreted as CRT achieving competitive robustness with a simpler and more computationally efficient training procedure, rather than a universal superiority claim. Figure~\ref{fig:model_comparison} provides a visual comparison of the clean and mean corrupted accuracy across all three models.
...
\midrule
Baseline        & 86.4\%       & 80.9\%       & 74.9\%       & 67.3\% \\
AugMix          & 92.1\%       & 90.2\%       & 88.0\%       & 83.4\% \\
CRT (Proposed)  & 92.4\%       & 91.6\%       & 90.1\%       & 87.8\% \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
Baseline (CE only)              & 94.3\%              & 73.1\%       \\
$+$ Corrupted CE (no KL)        & 93.0\%              & 76.1\%       \\
$+$ KL (no warm-up)             & 92.1\%              & 76.8\%       \\
\textbf{CRT (KL + warm-up)}     & \textbf{93.7\%}     & \textbf{88.9\%} \\
\bottomrule
\end{tabular}
\end{table}
...
\paragraph{Effect of warm-up.} Adding the warm-up schedule recovers the clean accuracy to 93.7\% while boosting mCA to 88.9\%. The jump from 76.8\% (KL without warm-up) to 88.9\% (full CRT) confirms that the warm-up is not merely a stabilisation trick but a substantive contributor to final robustness: it allows the KL consistency pressure to act on representations that are already class-discriminative, so that the alignment signal reinforces meaningful structure rather than distorting it.
...
\midrule
\textbf{Mean}   & \textbf{73.1\%} & \textbf{86.2\%} & \textbf{88.9\%} \\
\bottomrule
\end{tabular}
\end{table}
